...
def apply_sorry_bench_judge_template(prompts, responses, model_name, prompt_method, judge_file):
    ...
    if model_name in ["llama-2-7b-chat", "llama-guard-7b", "ft-llama-2-7b-chat", "ft-llama-2-13b-chat", "md-judge-v0.1"]:
        ...
        # Degenerate-loop generations (e.g. translate-ml row 3) can blow past
        # the judge's 32K context. Truncate the answer with a token-aware
        # binary search so the rendered dialog fits.
        from transformers import AutoTokenizer
        _judge_tok = AutoTokenizer.from_pretrained(get_model_path(model_name))
        _MAX_JUDGE_PROMPT_TOKENS = 32768 - 1024 - 64
        _n_truncated = 0
        ...
        for prompt, response in zip(prompts, responses):
            ...
            if SYSTEM_PROMPT != "":
                ...
                INPUT_TEMPLATE = PROMPT_TEMPLATE
            question_text = prompt["turns"][0]
            answer_text = response["choices"][0]["turns"][0]

            def _build(ans):
                return B_INST + " " + INPUT_TEMPLATE.format(question=question_text, answer=ans).strip() + " " + E_INST

            dialog = _build(answer_text)
            if len(_judge_tok.encode(dialog, add_special_tokens=False)) > _MAX_JUDGE_PROMPT_TOKENS:
                lo, hi, best = 0, len(answer_text), ""
                while lo < hi:
                    mid = (lo + hi + 1) // 2
                    if len(_judge_tok.encode(_build(answer_text[:mid]), add_special_tokens=False)) <= _MAX_JUDGE_PROMPT_TOKENS:
                        best, lo = answer_text[:mid], mid
                    else:
                        hi = mid - 1
                dialog = _build(best + " ... (truncated for judge context)")
                _n_truncated += 1
            dialogs.append(dialog)

        if _n_truncated:
            print(f"[judge] truncated {_n_truncated}/{len(dialogs)} prompts to fit 32K context")
